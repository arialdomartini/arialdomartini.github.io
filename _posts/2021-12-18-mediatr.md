...
Those practices do resonate with me. Indeed, this seems to be something that should be completely obvious, the only and correct way to build software.<br/>
There is no doubt that the MediatR library is very successful and widely adopted among the developers that share these values.

While there are many articles about it, only a minority of them focus on the drawbacks and help identify the cases where using it doesn't make much sense.<br/>
This post is one of them.

In this article I wish to talk about when and why the adoption of MediatR might fail to deliver the expected outcome. In fact I think that, despite the good intentions, some ways of integrating MediatR can be detrimental in the pursue of the best design practices.

Lastly, I want to make it clear that my criticism is about MediatR the library, not about the design principles it is supposed to support.<br/>
...
* Good news is: **MediatR can be easily replaced with trivial OOP techniques**, which is about to 50x faster and allocates way less memory.
...
In his benchmarks Adam measured that MediatR allocated over 1.67 GB of memory in `1'` of execution time, with over 2 seconds of GC time.
