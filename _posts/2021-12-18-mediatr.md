...
Those practices do resonate with me. Indeed, this seems to be something that should be completely obvious, the only and correct way to build software. There is no doubt that the MediatR library is very successful and widely adopted among the developers who share these values.
...
Under this light, I tend to agree with [@grauenwolf][grauenwolf]'s obversvation that:

> MediatR has always been just an expensive way to do a normal function call.
...
While the the latter can only send `Ping` messages via `IPingHandler`, the former might emit the same `Ping` messages for `IPingHandler`, but also a `Foo` message for `IFooHandler` or any other message: dependencies just cannot be inferred from its interface.

This is the very same issue as injecting a Service Locator: it changes the dependencies graph of a system from
...
Please, notice how in the first diagram, components are not coupled: they can easily depend abstractions by using interfaces, and therefore be decoupled as they should.

I took these 2 diagrams from [Are you using MediatR?][using-mediatr] by [Nam Duong][nam]. I suggest you to stop reading this post right away and get 3 minutes to check Nam's article out. It's the best, most concise and clearest explanation on this topic you can possibly find.

That MediatR is in fact a Service Locator should not come as a surprise: despite being largely considered an anti-pattern (see [Mark Seemann - Service Locator is an Anti-Pattern][service-locator]), the MediatR author believes it's not and sees no problems using it in these circumstances.
...
The compiler has no way to help you making sure that a Request class is properly handled. Checking if a `PingRequest` is handled by 0, 1 or more Request Handlers can only be done at runtime. You will probably need to mitigate this problem with some extra unit tests, because you cannot count on compilation errors.
...
In his benchmarks Adam measured that MediatR allocated over 1.67 GB of memory in 1 minute of execution, with over 2 seconds of GC time.
...
# Conclusion
MediatR is not an implementation of the Mediator Pattern: it is instead an in-process bus, largely based on the Service Locator pattern. It creates a coupling between your business code and interfaces externally defined, and in most of the cases an unjustifiable extra layer of indirection.<br/>
...
The chances are you can successfully replace it with a plain, old interface.<br/>
