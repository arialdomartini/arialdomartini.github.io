...
## Reality
Now, as Kent Beck says in [TDD isn't design][tdd-isnt-design]:

> nobody with any sense says TDD replaces the need for design

In fact, *Emergent Design*, in this context, has more the meaning
discussed in the C2 Wiki [Can An Architecture
Emerge][cs-architecture-emerge]:

> emergent means emerging from the system 
> rather than being imposed on the system.
...
The Print Diamond Kata is a very interesting exercise in that, if you
...
Tests will help *challenging* your solution, not *conceiving* it.
...
case working, hardcoding the implementation, then to follow with `B`
and then `C` etc.

As Alistair Cockburn writes in [Thinking before
...
of the way we classically apply TDD. 

Once we revisit some of the canonical assumptions, it is possible
to have emergent design.  
